        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
            ...
            <p className="max-w-sm text-xs text-muted-foreground">
              Start your first root cause investigation by clicking the button below. Each investigation generates a
              full RCFA report with citations to any SOP or manual you attach.
            </p>
            <button
                ...
                {items.map((inv) => (
                  <tr key={inv.id} className="transition-colors hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3 text-xs text-muted-foreground">
                      {inv.dateTime ? new Date(inv.dateTime).toLocaleDateString() : '—'}
                    </td>
                    <td className="px-4 py-3 font-medium">{inv.equipment}</td>
                    <td className="px-4 py-3 text-muted-foreground">{inv.labName}</td>
                    <td className="px-4 py-3"><SeverityBadge severity={inv.severity} /></td>
                    <td className="px-4 py-3"><StatusBadge status={inv.status} /></td>
                    <td className="px-4 py-3 text-right">
                      <Link
                        to={`/investigation/${inv.id}`}
                        className="inline-flex items-center gap-1 rounded-md px-2.5 py-1 text-xs font-medium text-primary hover:bg-primary/10"
                      >
